...
/**
  * Money stored in minor units (e.g. cents). Negative values represent outflows. No further
  * constraint — the sign carries meaning.
  */
...
/**
  ...
  * want a non-blank, bounded String so a malformed row can't sneak past the DB boundary.
  */
type PasswordHash = PasswordHash.T
object PasswordHash extends RefinedType[String, Not[Blank] & MaxLength[120]] {}

/**
  * Plaintext password coming in over the wire. 8..200 chars; everything else is the hashing
  * function's problem.
  */
