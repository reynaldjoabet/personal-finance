...
/**
  * Circe codecs. `iron-circe` derives codecs for the refined opaque newtypes automatically via
  * `RefinedType.Mirror`.
  */
object Json {
  ...
  given Decoder[YearMonth] =
    Decoder.decodeString.emap(s =>
      scala.util.Try(YearMonth.parse(s)).toEither.left.map(_.getMessage)
    )
  ...
  /**
    * What clients get from `GET /truelayer/connect`: the authorize URL to redirect the user to,
    * plus the CSRF state we'll expect echoed back on the callback.
    */
