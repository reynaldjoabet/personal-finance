...
/** TrueLayer client.
  ...
  *
  * Real-world docs: https://docs.truelayer.com/. The Iron-refined domain types meet untrusted upstream data inside
  * `toDomainAccount` / `toDomainTransaction`.
  */
trait TrueLayer[F[_]] {
...
object TrueLayer {
  ...
  /** Token endpoint response (the bits we care about). TrueLayer follows the standard RFC 6749 shape. */
  ...
  def fromHttp[F[_]: Concurrent](client: Client[F], cfg: TrueLayerConfig): TrueLayer[F] = {
    ...
    new TrueLayer[F] {
      ...
      def listAccounts(token: AccessToken): F[List[RawAccount]] = {
        val req = Request[F](Method.GET, apiBase / "data" / "v1" / "accounts").putHeaders(auth(token))
